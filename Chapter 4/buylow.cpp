...
{
	long long rem = 0;
	//1000 = MAX string length
	string result; result.resize(1000);
...
//longest[x] - longest decreasing subsequences starting at x
int longest[5001];
//sequences[x] - number of unique longest decreasing subsequences starting at x
BigInteger sequences[5001];
int prices[5000];
...
int main(){
	...
	BigInteger total_sequences("1");
	int longest_sequence = 1;

	longest[n - 1] = 1;
	sequences[n - 1] = BigInteger("1");
	unordered_set<int> max_dupes;
	for (int i = n - 2; i >= 0; --i) {
		int max_val = 0;
		BigInteger max_sequences("1");
		unordered_set<int> dupes;

		for (int j = i + 1; j < n; ++j) {
			if (prices[j] >= prices[i])
				continue;
			if (longest[j] > max_val) {
				max_val = longest[j];
				max_sequences = sequences[j];
				dupes.clear();
				dupes.insert(prices[j]);
			}
			else if (longest[j] == max_val && dupes.find(prices[j]) == dupes.end()) {
				max_sequences += sequences[j];
				dupes.insert(prices[j]);
			}
		}
		longest[i] = max_val + 1;
		sequences[i] = max_sequences;

		if (longest[i] > longest_sequence) {
			longest_sequence = longest[i];
			total_sequences = max_sequences;
			max_dupes.clear();
			max_dupes.insert(prices[i]);
		}
		else if (longest[i] == longest_sequence && max_dupes.find(prices[i]) == max_dupes.end()) {
			total_sequences += max_sequences;
			max_dupes.insert(prices[i]);
		}
	}

	fout << longest_sequence << " " << (string)total_sequences << endl;
